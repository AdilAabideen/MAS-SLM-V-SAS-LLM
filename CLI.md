...
Without `--fixture`, model construction uses the explicitly registered providers and the environment variables named in the experiment YAML. The CLI reads the nearest `.env` while walking from the experiment file's directory toward your home directory, so a `.env` in the repository root or its parent works without `export`. Shell-exported variables override values in that file. The resolved values are also used when constructing live providers and configuring telemetry; `validate` and `inspect` make no model requests. With `--fixture`, only the fixture's dummy environment is used, and no model requests are sent. All runs use a deterministic character-based token estimate by default, so a fresh offline clone never downloads a tokenizer for telemetry. Code that explicitly constructs `TokenEstimator(prefer_tiktoken=True)` can use a locally available tiktoken encoding; cache or network failures fall back to character estimation. Reports identify estimated usage separately from provider-reported usage.

Human progress from `run` and `compare` streams to stderr as each event occurs, while result JSON stays on stdout until the command finishes. Choose `--console none|summary|events|full` and `--color auto|always|never`, or use the experiment's reporting settings. [CONSOLE_TRACES.md](CONSOLE_TRACES.md) describes event order, redaction, and renderer failure behavior.
